...
#pragma warning( \
    disable : 4996)  // Disable deprecated std::ctype<char>::widen, std::copy
#endif
...
// template <class Ch>
// BOOST_DEPRECATED(
//     "Use Ch[37] instead of Ch[36] to allow for the null terminator")
...
std::basic_ostream<Ch, Traits>& operator<<(std::basic_ostream<Ch, Traits>& os,
                                           uuid const& u) {
  char tmp[37];
  to_chars(u, tmp);
...
template <class Ch, class Traits>
std::basic_istream<Ch, Traits>& operator>>(std::basic_istream<Ch, Traits>& is,
                                           uuid& u) {
