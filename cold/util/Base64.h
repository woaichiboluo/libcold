#ifndef COLD_UTIL_BASE64
#define COLD_UTIL_BASE64
...
inline std::string Base64Decode(std::string_view input) {
  ...
  while (ptr < end) {
    ...
    *writeBuf++ = static_cast<char>((packed >> 16) & 0xff);
    if (padding != 2) {
      *writeBuf++ = static_cast<char>((packed >> 8) & 0xff);
    }
    if (padding == 0) {
      *writeBuf++ = static_cast<char>(packed & 0xff);
    }
  }
